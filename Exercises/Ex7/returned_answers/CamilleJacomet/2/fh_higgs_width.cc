// modified from FeynHiggs demo.cc
...
int main() {
  ...
  ofstream out("fh_width.dat");

  int error;
  double MHiggs[4];
  double GammaTot[4];

  for (double MA0 = 100; MA0 <= 200; MA0 += 10) {
    // either use setPara to set the parameters directly
    // or use setSLHA to read them from an SLHA file
      setPara(MA0);
    //  setSLHA("myfile.SLHA");

    FHHiggsCorr(&error, MHiggs, nullptr, nullptr, nullptr);
    if (error) exit(error);

    // This is not an existing function, but I don't know how to get the width
    // the FeynHiggs website does not work, so I couldn't find more info there
    FHHiggsWidth(&error, GammaTot);
    if (error) exit(error);

    cRealType mh = MHiggs[0];
    cRealType width = GammaTot[0];
